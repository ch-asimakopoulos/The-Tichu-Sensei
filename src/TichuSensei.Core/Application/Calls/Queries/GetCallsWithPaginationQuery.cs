...
{
    /// <summary>
    /// Returns a portion of the total Calls using Pagination. The page number, page size and order direction can be specified. Ordering will be done by name.
    /// </summary>
    public class GetCallsWithPaginationQuery : IRequest<PaginatedList<CallDTO>>
    {
    ...
    public class GetCallsWithPaginationQueryHandler : IRequestHandler<GetCallsWithPaginationQuery, PaginatedList<CallDTO>>
    {
        private readonly IApplicationDbContext _context;
        private readonly IMapper _mapper;

        public GetCallsWithPaginationQueryHandler(IApplicationDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }

        public async Task<PaginatedList<CallDTO>> Handle(GetCallsWithPaginationQuery request, CancellationToken cancellationToken)
        {

            IQueryable<CallDTO> CallsNoTracking = _context.Calls.AsNoTracking().ProjectTo<CallDTO>(_mapper.ConfigurationProvider);
